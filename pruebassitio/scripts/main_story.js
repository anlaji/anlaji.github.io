...
function createSillyStory() {
  ...
  story.textContent = '';
  let newStoryText = '';
  newStoryText = storyText.replaceAll(':insertx:', randomValueFromArray(insertX).toString());
  newStoryText = newStoryText.replaceAll(':inserty:', randomValueFromArray(insertY));
  newStoryText = newStoryText.replaceAll(':insertz:', randomValueFromArray(insertZ));
  if(language.checked){
